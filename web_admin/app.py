...
def api_perfil_password():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('UPDATE usuarios SET password_hash = ? WHERE id = ?', (new_hash, usuario['id']))
    conn.commit()
    conn.close()
...
def api_crear_trabajador():
    ...
    for modulo_nombre in modulos:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM modulos WHERE nombre = ?", (modulo_nombre,))
        mod = cursor.fetchone()
        conn.close()
        if mod:
            asignar_permiso_usuario(user_id, mod['id'], 1)
...
def api_subir_foto_producto(producto_id):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('SELECT negocio_id FROM productos WHERE id = ?', (producto_id,))
...
def api_eliminar_foto_producto(producto_id):
    ...
    # Eliminar la referencia a la foto
...
def api_productos_tienda():
    ...
    for p in productos:
        result.append({
            ...
            'precio': p['precio'],
            'stock': p['stock'],
            'foto_url': p['foto_url'] or '/static/img/default_product.png'
        })
...
def api_ventas():
    ...
    if tipo == 'producto' and producto_id:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('SELECT stock FROM productos WHERE id = ? AND negocio_id = ?', (producto_id, usuario['id']))
...
def api_ventas_estadisticas():
    ...
    return jsonify({
        ...
        'ingresos': stats['ingresos'] if stats else 0
    })
